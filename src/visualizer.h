...
public:
  GeneticVisualizer(std::vector<generation> &gens) : generations(gens) {
    ...
    windowWidth =
        glutGet(GLUT_SCREEN_WIDTH) * 0.8; // 80% del ancho de la pantalla
    windowHeight =
        glutGet(GLUT_SCREEN_HEIGHT) * 0.8; // 80% del alto de la pantalla
    barWidth = static_cast<float>(windowWidth) / gens.size();

    maxFunctionValue = pow(xlimit - 1, 2) + 2 * (xlimit - 1) * (ylimit - 1) +
                       pow(ylimit - 1, 2);
  }

  static void displayCallback() {
    glClear(GL_COLOR_BUFFER_BIT);
    GeneticVisualizer *visualizer =
        static_cast<GeneticVisualizer *>(glutGetWindowData());
    visualizer->draw();
    glutSwapBuffers();
  }
  ...
  void draw() {
    ...
    for (int i = 0; i <= segments; i++) {
      ...
      glRasterPos2f(
          -40,
          yPosition - 5); // Ajustar la posición según el tamaño de la etiqueta
      for (char c : label) {
    ...
    for (size_t i = 0; i < generations.size(); i++) {
      float x = i * barWidth +
                barWidth / 2; // Centramos los puntos en el medio de cada barra
      glColor3f(0.0, 1.0, 0.0); // Color azul para otros valores
      glBegin(GL_POINTS);
      glVertex2f(x, (generations[i].average / maxFunctionValue) * windowHeight);
      glEnd();
      for (int j = 0; j < t; j++) {
        float y = (static_cast<float>(generations[i].function[j]) /
                   maxFunctionValue) *
                  windowHeight;
  ...
  void run() {
    ...
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluOrtho2D(0, windowWidth, 0,
               windowHeight); // Establecer una proyección ortográfica
