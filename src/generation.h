...
struct generation {
  ...
  void initializer() // zeros and randoms
  {
    ...
    for (int i = 0; i < t; i++) {
      oldx[i] = oldy[i] = partner[i] = crossposx[i] = crossposy[i] = x[i] =
          y[i] = function[i] = nextx[i] = nexty[i] = 0;
  ...
  void adjust_actual() {
    ...
    if (current_sum < t) {
      // Si la suma es menor que t, ajustar los valores de actual basados en las
      // diferencias de la parte decimal
      while (current_sum < t) {
      ...
      // Si la suma es mayor que t, ajustar los valores de actual basados en las
      // diferencias de la parte decimal
      while (current_sum > t) {
  ...
  void oldbins() {
    ...
    for (int i = 0; i < t; i++) {
      x_thread[i] = thread(&generation::convertToBinary, this, oldx[i],
                           old_xbin[i], xbits);
      y_thread[i] = thread(&generation::convertToBinary, this, oldy[i],
                           old_ybin[i], ybits);
    }
  ...
  void print() {
    cout << "------------------------------------------------------------------"
            ...
            "-------------------------------------------------------"
         << endl;
    cout << "| OldX | OldY |   Old_Xbin   |   Old_Ybin   | Partner | CrossPosX "
            "| CrossPosY |   Xbin   |   Ybin   |  X  |  Y  | Equation |   "
            "Selected   |   Spected   |   Actual   |   NextGen   |"
         << endl;
    cout << "------------------------------------------------------------------"
            "------------------------------------------------------------------"
            "-------------------------------------------------------"
    ...
    for (int i = 0; i < t; i++) {
      ...
      for (int j = 0; j < ybits; j++)
        cout << ybin[i][j];
      cout << "  ||  ";
      cout << x[i] << "  |  " << y[i] << "  ||  " << function[i] << "  ||  "
           << selected[i] << "  |  " << spected[i] << "  |  " << actual[i]
           << "  ||  " << nextx[i] << "  |  " << nexty[i] << "\n";
    }
